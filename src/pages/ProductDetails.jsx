    ...
    <div className="detail-wrap">
      <div className="detail-card fade-in">
          ...
          {product.images && product.images.length > 1 && (
            <div className="thumbs">
              {product.images.map((img, idx) => (
                <button key={idx} className={`thumb-btn ${activeImg === img ? "ring-2 ring-teal-300" : ""}`} onClick={() => setActiveImg(img)}>
        ...
        <div className="detail-info">
          ...
          <div className="detail-desc mt-3">{product.description}</div>

          <div className="detail-actions">
            <button className="btn-accent" onClick={() => addToCart(product)}>Add to Cart</button>
            <button className="btn-primary" onClick={() => { addToCart(product); alert("Proceed to checkout (demo)"); }}>Buy Now</button>
          </div>
        </div>
      </div>
    </div>
